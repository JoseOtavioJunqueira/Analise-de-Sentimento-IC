...
ARQUIVO_ENTRADA = 'noticias_mapeadas.json'
ARQUIVO_SAIDA = 'analise_frequencia.json'
ARQUIVO_MAPA = 'mapeamento_tickers.json'  # Adicionamos o mapa aqui!
...
def processar_frequencia():
    ...
    # 1. Carregar o Dicionário Reverso (Ticker -> Nome da Empresa)
    ticker_para_nome = {}
    if os.path.exists(ARQUIVO_MAPA):
        with open(ARQUIVO_MAPA, 'r', encoding='utf-8') as f:
            mapa = json.load(f)
            for nome_empresa, ticker in mapa.items():
                if ticker and ticker not in ticker_para_nome:
                    # Salva o primeiro nome associado ao ticker
                    ticker_para_nome[ticker] = nome_empresa
    else:
        print(f"Aviso: {ARQUIVO_MAPA} não encontrado. Nomes genéricos serão usados.")

    # 2. Ler as notícias processadas
    with open(ARQUIVO_ENTRADA, 'r', encoding='utf-8') as f:
        try:
            noticias = json.load(f)
        except json.JSONDecodeError:
            print("Erro: Falha ao ler o JSON de entrada.")
            return
    ...
    for ticker, qtd in contagem_ticker.items():
        resultado["estatisticas"].append({
            "ticker": ticker,
            "empresa": ticker_para_nome.get(ticker, "Desconhecida"), # Puxa do dicionário oficial
            "vezes_que_apareceu": qtd,
            "nivel_informacao": "ALTO" if qtd >= 100 else "MEDIO" if qtd >= 50 else "BAIXO"
        })
